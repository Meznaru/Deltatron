...
public:
  static json_container_imp parse_stream(std::string const& id, std::string_view data)
...
private:
  ...
  : _begin(!ts.empty() && ts.begin()->type() == json_t::LBrace ? ts.data() + 1 : ts.data()),
    _end(ts.data() + ts.size())
  { json_semantic_analysis::verify_stream(id, ts); }

  json_container_imp _create_root_object() const {
  ...
  json_type _get_index(iter_t& t) const {
    switch (t->type()) {
    case json_t::LBrace: return _create_object(++t);
    case json_t::LBrack: return _create_array(++t);
    default:
      return *t;
    }
  }

  json_container_imp _create_object(iter_t& t) const {
  ...
  json_container_imp _create_array(iter_t& t) const {
    json_array_type arr{};
